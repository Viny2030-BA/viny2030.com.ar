// routes/orders.js — v3 con proxy GitHub para repos privados
...
// ─────────────────────────────────────────────────────────────────
// POST /api/orders/analisis-ia — Proxy IA
// Recibe { prompt } y llama a Anthropic desde el servidor
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// GET /api/orders/:code/github-files
// Proxy backend para leer archivos del repo PRIVADO del cliente
// El browser no puede acceder repos privados sin token
// ─────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    try {
      ...
      if (r.ok) {
        ...
        files = items
          .filter(f => f.name.match(/\.(csv|xlsx|xls|parquet|json|txt)$/i))
...
// ─────────────────────────────────────────────────────────────────
// GET /api/orders/:code/github-file?path=relato/archivo.csv
// Proxy backend para leer CONTENIDO de archivo del repo privado
// ─────────────────────────────────────────────────────────────────
router.get('/:code/github-file', async (req, res) => {
  try {
    ...
    res.json({ name: d.name, content, size: d.size });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/:code/analisis', async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE orders SET analisis_es=$1, analisis_trad=$2, propuesta_es=$3, propuesta_trad=$4, analisis_at=NOW(), status='analizado' WHERE id=$5`,
...
router.post('/:code/aceptar', async (req, res) => {
  try {
    ...
    const cbuPesos=process.env.CBU_PESOS||'0140005203400552652310', aliasPesos=process.env.ALIAS_PESOS||'ALGORIT.MONTE.PESOS';
    const cbuDolares=process.env.CBU_DOLARES||'0140005204400550329709', aliasDolares=process.env.ALIAS_DOLARES||'ALGO.MONTE.DOLARES';
    const titular=process.env.TITULAR||'Vicente Humberto Monteverde', cuit='20-12034411-1';
    const bancoIntl=process.env.BANCO_INTERNACIONAL||'Banco Santander Montevideo', cuentaIntl=process.env.CUENTA_INTERNACIONAL||'005200183500';
    const dir=process.env.DIRECCION_BENEFICIARIO||'Av. Directorio 3024-PB-Dto 04', swift=process.env.SWIFT||'BSCHUYMM';
    ...
    await sendEmail({ to: process.env.ADMIN_EMAIL||process.env.GMAIL_USER, subject:`ACEPTACION: ${order.id} - ${order.name} - USD 40 pendiente`, html:`<div style="font-family:Arial;padding:20px;"><h2 style="color:#c9a84c;">Cliente aceptó el planteo</h2><p><b>Orden:</b> ${order.id}</p><p><b>Cliente:</b> ${order.name} (${order.email})</p><p><b>Estado:</b> Aceptado — esperando pago USD 40</p><p><b>Fecha:</b> ${order.aceptado_at}</p></div>` });
