// routes/orders.js — v4 fusionado: rutas originales + segunda etapa GitHub
...
router.post('/:code/aceptar', async (req, res) => {
  try {
    ...
    await sendEmail({ to: process.env.ADMIN_EMAIL||process.env.GMAIL_USER, subject:`ACEPTACION: ${order.id} - ${order.name} - USD 40 pendiente`, html:`<div style="font-family:Arial;padding:20px;"><h2 style="color:#c9a84c;">Cliente aceptó el planteo</h2><p><b>Orden:</b> ${order.id}</p><p><b>Cliente:</b> ${order.name} (${order.email})</p><p><b>Estado:</b> Aceptado — esperando pago USD 40</p></div>` });
...
router.post('/:code/pago2', async (req, res) => {
  ...
  try {
    ...
    if (order.status !== 'aceptado') return res.status(400).json({ error: `Estado inválido. Esperado: aceptado. Actual: ${order.status}` });
...
router.get('/:code/repo-status', async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ error: 'Orden no encontrada' });
    res.json({ repo_name: order.repo_name || null, repo_url: order.repo_url || null, repo_created_at: order.repo_created_at || null, status: order.status });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
