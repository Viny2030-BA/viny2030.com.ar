// routes/orders.js — con proxy IA integrado
      ...
      try {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: 'user', content: `Translate to ${LANG_NAMES[order.lang] || order.lang}. Return ONLY JSON with keys analisis and propuesta.\nANALISIS: ${analisis}\nPROPUESTA: ${propuesta}` }]
          })
        });
        const data = await response.json();
    ...
    const html = `
      <div style="font-family:Arial;max-width:600px;margin:0 auto;background:#0a0a0a;color:#f0f0f0;padding:30px;border-radius:10px;">
        ...
        <div style="background:#111;border-left:4px solid #c9a84c;padding:16px;margin:20px 0;">
          <h3 style="color:#c9a84c;">${g.at}</h3><p style="color:#ddd;">${analisisTraducido}</p>
        </div>
        <div style="background:#111;border-left:4px solid #e94560;padding:16px;margin:20px 0;">
          <h3 style="color:#e94560;">${g.pt}</h3><p style="color:#ddd;">${propuestaTraducida}</p>
        </div>
