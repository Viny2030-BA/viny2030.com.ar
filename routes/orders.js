// routes/orders.js — con proxy IA integrado y correcciones v2
...
// CORRECCIÓN: recibe { prompt } y llama a Anthropic correctamente
// ─────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: `Error Anthropic API: ${response.status}`,
        detail: errText
      });
    }
...
router.get('/:code', async (req, res) => {
  try {
    ...
    res.json({
      ...
      uploadUrl: o.upload_url, createdAt: o.created_at,
      aceptadoAt: o.aceptado_at,
      analisis: o.analisis_es
        ? { es: o.analisis_es, traducido: o.analisis_trad || o.analisis_es }
        : null,
      propuesta: o.propuesta_es
        ? { es: o.propuesta_es, traducido: o.propuesta_trad || o.propuesta_es }
        : null
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.post('/:code/analisis', async (req, res) => {
  try {
    ...
    // Traducción automática si el cliente no es hispanohablante
      ...
      try {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            ...
            messages: [{
              role: 'user',
              content: `Translate to ${LANG_NAMES[order.lang] || order.lang}. Return ONLY JSON with keys "analisis" and "propuesta". Keep technical terms. No extra text.\nANALISIS: ${analisis}\nPROPUESTA: ${propuesta}`
            }]
          })
        });
        const data = await response.json();
        ...
        // Continúa con el texto en español si falla la traducción
      }
    }

    await pool.query(
      `UPDATE orders
       SET analisis_es = $1, analisis_trad = $2, propuesta_es = $3, propuesta_trad = $4,
    ...
    const html = `
      <div style="font-family:Arial;max-width:600px;margin:0 auto;background:#0a0a0a;color:#f0f0f0;padding:30px;border-radius:10px;">
        ...
        <div style="text-align:center;padding:20px;background:#1a1a1a;border-radius:8px;">
          ...
          <a href="${aceptarUrl}"
             style="background:#c9a84c;color:#000;padding:14px 28px;text-decoration:none;border-radius:6px;font-weight:bold;">
            ${g.btn}
          </a>
    ...
    await sendEmail({
      to: order.email,
      subject: `Viny 2030 - Analisis ${order.id}`,
      html
    });
    await sendEmail({
      to: process.env.ADMIN_EMAIL || process.env.GMAIL_USER,
...
router.post('/:code/aceptar', async (req, res) => {
  try {
    ...
    const SUBJ = {
      es: `Viny 2030 - Aceptaste el planteo ${order.id} - USD 40`,
      en: `Viny 2030 - Proposal accepted ${order.id} - USD 40`,
      fr: `Viny 2030 - Proposition acceptée ${order.id} - USD 40`,
      de: `Viny 2030 - Vorschlag akzeptiert ${order.id} - USD 40`,
      it: `Viny 2030 - Proposta accettata ${order.id} - USD 40`
    };
    const HOLA  = { es: 'Hola', en: 'Hello', fr: 'Bonjour', de: 'Hallo', it: 'Ciao' };
    const INTRO = {
      es: 'Gracias por aceptar el planteo! A continuación los datos de pago (USD 40):',
      en: 'Thank you! Here are the payment details (USD 40):',
      fr: 'Merci! Voici les coordonnées de paiement (USD 40):',
      de: 'Danke! Hier sind die Zahlungsdetails (USD 40):',
      it: 'Grazie! Ecco i dati di pagamento (USD 40):'
    };
    const PLAZO = { es: 'Plazo: 72 horas', en: 'Deadline: 72 hours', fr: 'Délai: 72 heures', de: 'Frist: 72 Stunden', it: 'Scadenza: 72 ore' };
    const CIERRE = {
      es: 'Una vez recibido el pago coordinamos la implementación.',
      en: 'Once payment is received we coordinate implementation.',
      fr: 'Dès réception du paiement, nous coordonnons.',
      de: 'Nach Zahlungseingang koordinieren wir.',
      it: 'Una volta ricevuto il pagamento coordinamo.'
    };

    const row = (k, v) =>
      `<tr><td style="padding:5px 10px;color:#888;font-size:12px;">${k}</td><td style="padding:5px 10px;color:#e8c96d;font-weight:bold;font-size:12px;">${v}</td></tr>`;
    ...
    await sendEmail({
      ...
      html: `<div style="font-family:Arial;padding:20px;">
        <h2 style="color:#c9a84c;">Cliente aceptó el planteo</h2>
        <p><b>Orden:</b> ${order.id}</p>
        <p><b>Cliente:</b> ${order.name} (${order.email})</p>
        <p><b>Estado:</b> Aceptado — esperando pago USD 40</p>
        <p><b>Fecha:</b> ${order.aceptado_at}</p>
      </div>`
    });
